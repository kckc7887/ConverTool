...
29. [Host / 插件 `config.json`（`SettingManager`）](#29-host--插件-configjsonsettingmanager)  
...
1. **初始化服务定位器**  
   ...
   - 创建 `I18nService`、`PluginI18nService`、`PluginCatalog` 单例。
   - `PluginCatalog.LoadFromOutput` 扫描 `plugins/` 目录下的 manifest.json。
...
4. **`AppServices` 静态门脸**（`Host/AppServices.cs`）  
   - `I18n`：通过 `ServiceLocator.GetService<I18nService>()` 获取。
   - `Plugins`：通过 `ServiceLocator.GetService<PluginCatalog>()` 获取。
   - `PluginI18n`：通过 `ServiceLocator.GetService<PluginI18nService>()` 获取。
   - `ReloadPlugins(baseDir)`：调用 `ServiceLocator.ReloadPluginCatalog(baseDir)` 重新扫描插件目录。
...
**与 `PluginAbstractions.PluginManifest` 的差异**：代码里 `GetManifest()` 返回的记录类型**字段更少**；**运行时以 JSON manifest 为准**（配置 schema 扩展、`visibleIf`、`visibleForInputExtensions`、`visibleForTargetFormats` 等）。
...
- 维护 **`ObservableCollection<InputFileItemVm> InputFiles`**（拖拽/添加/移除），通过 **`InputFileViewModel`** 子 ViewModel 管理。
...
**子 ViewModel**：
- **`InputFileViewModel`**：管理输入文件列表、拖拽/浏览逻辑、不支持格式检测。
- **`NamingTemplateViewModel`**：管理命名模板标签、自定义标签文本、标签状态计算。
- **`ConfigFields`**：配置字段 ViewModel 集合，按 manifest `configSchema` 动态生成。
...
## 10. UI 规则引擎：`visibleIf`、`visibleForInputExtensions`、`visibleForTargetFormats`、`fieldBoolRelations`、目标格式过滤

**状态字段**：`_visibleIfByFieldKey`、`_visibleForInputExtensionsByFieldKey`、`_visibleForTargetFormatsByFieldKey`、`_fieldBoolRelations`、`_allTargetFormatModels`。
...
2. **`RefreshTargetFormatsByVisibility()`**：对 `_allTargetFormatModels` 逐项应用 `inputExtensions` / 目标级 `visibleIf`；重建 `TargetFormats`；若当前选中 id 不在可见列表则回退到第一项。**先于配置字段可见性计算**，以便 `visibleForTargetFormats` 使用稳定的 `SelectedTargetFormat`。

3. **配置字段可见性**：对每个 `ConfigFieldVm` 依次应用 **`visibleIf`**（控制字段须为复选框）、**`visibleForInputExtensions`**（当前输入扩展名）、**`visibleForTargetFormats`**（当前选中目标格式 id，来自 `SelectedTargetFormat`）。

4. **`ApplyTargetSizeUnitConversionIfNeeded` / `UpdateTargetSizeLabels`**：与图片插件 `targetSizeUnit` 联动时，在切换 KB/MB 时换算数值并刷新标签括号内单位（见第 12 节）。  

**`SelectedTargetFormat` 变更**：Setter 在非加载、非重入时调用 `ReevaluateUiRules(null)`，以便切换目标格式后立即刷新 `visibleForTargetFormats` 字段。

**目标格式选择器 UI**：使用 **`Button + Flyout + ListBox`** 组合替代传统 `ComboBox`，提供更好的用户体验：
- `Button` 显示当前选中的格式名称（`SelectedTargetFormatDisplayText`）
- 点击后弹出 `Flyout`，内部 `ListBox` 展示所有可选格式
- `ListBox.ItemTemplate` 显示两列：格式名称（`Text`）和格式 ID（`Id`）
- 选中项后自动关闭 Flyout（通过 `OnTargetFormatFlyoutListKeyDown` / `OnTargetFormatFlyoutListPointerReleased` 事件处理）
...
- **`UserSettingsFile`**：`locale`、`outputDir`、`useInputDirAsOutput`、`namingTemplate`、`customToken1Text`、`customToken2Text`、`customToken3Text`、`enableParallelProcessing`、`parallelism`、`keepTemp`、`plugins[pluginId]`（`targetFormatId` + `fields` 字典）。  
...
**占位符**：`MoveToFinalOutput` 替换 **`{base}`、`{ext}`、`{index}`、`{timeYmd}`、`{timeHms}`、`{custom1}`、`{custom2}`、`{custom3}`**：
- `{base}` → 原文件名（不含扩展名）
- `{ext}` → 目标格式扩展名
- `{index}` → 文件序号（从 1 开始）
- `{timeYmd}` → 当前日期（yyyy-MM-dd）
- `{timeHms}` → 当前时间（HH-mm-ss）
- `{custom1}`/`{custom2}`/`{custom3}` → 用户自定义文本（通过界面输入）

`ExecuteContext.OutputNamingContext` 提供键 **`base`、`index`、`ext`、`timeYmd`、`timeHms`**（后两者为字符串，与替换格式一致），供插件只读。

**`NormalizeNamingTemplate`**：
- 去掉历史写法中的 `{ext}` / `.{ext}`，因 UI 右侧固定展示目标后缀
- **空模板处理**：不再自动将空模板转换为 `{base}`，保持用户设置的空模板状态
- 保留用户的原始模板输入，仅去除多余的扩展名占位符

**`ValidTokens` 验证**：初始化时只接受预定义的七个标签（`{base}`、`{index}`、`{timeYmd}`、`{timeHms}`、`{custom1}`、`{custom2}`、`{custom3}`），无效标签被自动过滤。

**`NamingTemplateTokenVm.DisplayText`**：对 `{base}`/`{index}`/`{timeYmd}`/`{timeHms}` 显示本地化候选文案，对 `{custom1}`/`{custom2}`/`{custom3}` 显示用户自定义文本，其它值原样显示。

**自定义标签**：
- 三个自定义标签 `{custom1}`、`{custom2}`、`{custom3}` 允许用户自定义显示名称和替换文本
- 自定义标签文本通过 `SetCustomTokenTexts`/`GetCustomTokenTexts` 方法管理
- 自定义标签文本持久化在 **Host 根目录 `config.json`** 的 **`CustomToken1Text`** / **`CustomToken2Text`** / **`CustomToken3Text`** 项（经 `SettingManager`），与全局 Host 设置一并保存
- **Host i18n**（`Host/locales/<locale>.json`）：候选芯片在未填写用户文案时，使用 **`host/output/namingTemplateCustom1Default`** 等；自定义输入行标题 **`host/output/namingTemplateCustomRowLabel`**；三个输入框水印 **`host/output/namingTemplateCustom1Watermark`** 等。`ApplyNamingTemplateHostI18n()` 在启动与语言切换时调用，内部执行 `NamingTemplateViewModel.SetTokenTexts`、`SetCustomTokenDefaultLabels` 并刷新上述绑定属性
- 当用户输入文本后，候选按钮与已选标签展示会同步用户输入的文本
- 输出文件名替换：见 `MoveToFinalOutput` 中对 `{custom1}`… 的处理

**状态计算优化**：`UpdateActionCandidateStates` 使用单次遍历同时计算七个标签的存在状态，替代原来的多次 `Any` 遍历。

**保存机制**：
- 主窗口关闭时统一 **`SaveSettings()`**（见 `MainWindow.axaml.cs` `Closing`），不再使用定时防抖自动保存命名模板
- 构造函数中使用 `_loadingUserSettings` 标志防止初始化期间误触发持久化逻辑
...
- **命名模板相关键**（节选）：`host/output/templateLabel`、`host/output/namingTemplateTokenBaseText` / `Index` / `TimeDate` / `TimeHms`、`host/output/namingTemplateCustom1Default`～`Custom3Default`、`host/output/namingTemplateCustomRowLabel`、`host/output/namingTemplateCustom1Watermark`～`Custom3Watermark`、`host/output/templateHelp` 等；**不得**在 `MainWindow.axaml` 中硬编码中英文。  
- **插件**：`plugins/<pluginId>/<localesFolder>/<locale>.json`，同上；**manifest** 中 `labelKey` / `displayNameKey` 等必须在两种语言文件中都有条目，否则界面会显示 **key** 字符串。  
- **`PluginI18nService`**：按插件目录加载；回退 **`en-US`**；再不行显示 key。  
- **语言切换**：`ReloadHostStrings()` → `ApplyNamingTemplateHostI18n()` + 大量 `RaisePropertyChanged`，并 `ReloadPluginContext()` 以刷新插件文案。
...
- Host 与插件目录 **`config.json`**（`SettingManager`）：插件侧 **必须** 提供该文件并由 Host **始终读取**；`settings[]` 各项可有可选 **`persistValue`**（见第 29 节）。  
...
## 29. Host / 插件 `config.json`（`SettingManager`）

**文件**：`Host/Settings/SettingManager.cs`

### 路径与读取（插件为必需）

| 作用域 | 路径 |
|--------|------|
| Host | `Path.Combine(AppContext.BaseDirectory, "config.json")` |
| 插件 | **`<PluginEntry.PluginDir>/config.json`**（实现上通过 `PluginCatalog` 解析每个插件 manifest 所在目录，与仅按 `pluginId` 拼 `plugins/<id>/` 相比，可避免 **manifest 的 `pluginId` 与磁盘文件夹名不一致** 时读错文件） |

`LoadAllPluginSettings` / `SaveAllPluginSettings` 以 **`PluginCatalog`** 中的插件列表为准；每个插件目录均有一条配置记录（含空 **`settings`**）。对每个已安装插件，Host **都会**加载对应 **`config.json`**（文件不存在时得到空字典）。插件作者在发行包中 **必须** 提供 **`config.json`**（至少 `{"settings":[]}`），与 **`plugin-dev.md` §9** 一致。

与 **`%LocalAppData%/ConverTool/user-settings.json`**（`UserSettingsStore`）不同：`config.json` 随安装目录/插件目录分布，用于 Host 全局项与各插件配置字段的 **`settings[]`** 快照；两者在 `MainWindowViewModel` 中分别加载与保存。

### 数据模型

- 根对象 **`ConfigFile`**，含 **`settings`**：`SettingItem` 列表。  
- **`SettingItem`**：`key`、`value`、`default`（保存 Host 配置时若某项 `default` 为空，可与内置规范默认值合并，便于首次写出完整 JSON）、**`persistValue`**（**单项**上的可选布尔，见 `plugin-dev.md` §9）。

### `persistValue`

| 条件 | 行为 |
|------|------|
| 未写或 `true` | `SetValue` 用当前 UI 状态更新该项的 **`value`**，保存时写回文件。 |
| `false` | **仍从文件读取** `value` 供 `GetValue` / 插件字段恢复；**`SetValue` 不覆盖**该项的 `value`，保存后磁盘上该项保持原样。 |

插件作者面向说明见 **[`plugin-dev.md` §9](../plugin-dev.md#9-configjsonhost-与插件目录)**。
